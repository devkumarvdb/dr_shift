...
class ReferralsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create referral" do
    assert_difference("Referral.count") do
      post referrals_url, params: { referral: { email: @referral.email, resend_count: @referral.resend_count, status: @referral.status, token: @referral.token, user_id: @referral.user_id } }
    end
  ...
  test "should update referral" do
    patch referral_url(@referral), params: { referral: { email: @referral.email, resend_count: @referral.resend_count, status: @referral.status, token: @referral.token, user_id: @referral.user_id } }
